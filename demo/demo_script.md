...
2. In the sidebar pick **CSV Upload**, then choose the bundled sample `cic2017_portscan_friday.csv` and click **Analyse**.
3. Press **▶ Play** at 2x.
...
4. The low-rate probing phase is labelled PortScan in the dataset. As it arrives, the scanner node (`172.16.0.1`) turns red-orange and the target (`192.168.10.50`) turns amber. Hot edges carry attack particles.
   - The attacker and victim roles are derived from the forecast plus traffic direction. They are not hardcoded.
5. At the first sustained alert, playback **pauses** on its own. The decision panel shows:
   - the stage (Reconnaissance, TA0043)
   - the recommended action from the rule engine (block the scanner)
   - its rationale
   - the integrated-gradients evidence (distinct ports, failed connections, and so on)
...
- `ctu13_botnet_scenario10.binetflow`: Rbot ICMP DDoS from 10 infected hosts
