...
(() => {
  ...
  async function livePageV2(content) {
    ...
    async function tick() {
      ...
      const recommendation = host.recommended_action || { action_type: actionForStage(host.stage), target_ip: host.host, target_port: 22, ttl_seconds: 300, rationale: "Review the current forecast before taking action.", requires_human_approval: true };
      const stageAction = recommendation.action_type || actionForStage(host.stage);
      ...
      content.innerHTML = card("", `<div class="nv-grid">${metric("Live hosts", num(data.n_hosts))}${metric("Alerting now", num(alerting), { cls: alerting ? "alert" : "" })}${metric("Feed freshness", `<small>${esc(fresh)}</small>`, { cls: stale ? "alert" : "ok" })}${metric("Pipeline", `<small>${esc(stale ? "STALE" : "LIVE")}</small>`, { cls: stale ? "alert" : "ok" })}${metric("Response mode", `<small>${esc(data.mode || "dry_run")}</small>`)}</div>`) +
        ...
        section("Live recommendation", recommendation.action_type ? "Recommended action now" : "Monitor only", recommendation.action_type ? "Generated from the current forecast; approval is required and nothing is applied automatically." : "No containment action is recommended while the current forecast is below the calibrated alert threshold.", `<div class="nv-recommendation"><div><b>${esc(recommendation.label || recommendation.action_type)}</b><span class="nv-badge ${recommendation.requires_human_approval ? "elevated" : "benign"}">${recommendation.requires_human_approval ? "REVIEW REQUIRED" : "MONITOR"}</span></div><p>${esc(recommendation.rationale || "")}</p>${recommendation.action_type ? `<p class="nv-note">Target: <span class="nv-mono">${esc(recommendationTarget)}${recommendationTargetPort ? `:${esc(recommendationTargetPort)}` : ""}</span> · TTL: ${esc(recommendation.ttl_seconds || 300)}s${recommendation.target_port_note ? ` · ${esc(recommendation.target_port_note)}` : ""}</p>` : ""}</div>`) +
        section("Human-in-the-loop response", "Preview a defensive action", "The form is populated from the current recommendation. Dry-run is the default; preview shows the exact target, rule and TTL before approval.", `<div class="nv-live-action-form"><label>Operator token <input id="nv-op-token" type="password" placeholder="required for approval" value="${esc(getToken())}"></label><label>Action <select id="nv-action-type">${actionOptions(stageAction)}</select></label><label>Target IP <input id="nv-action-ip" value="${esc(recommendationTarget)}"></label><label>Port <input id="nv-action-port" type="number" min="1" max="65535" value="${esc(recommendationTargetPort)}"></label><label>TTL seconds <input id="nv-action-ttl" type="number" min="1" max="3600" value="${esc(recommendation.ttl_seconds || 300)}"></label><label>Mode <select id="nv-action-mode"><option value="dry_run">dry-run</option><option value="enforce">enforce if server allows</option></select></label><button class="nv-btn" id="nv-action-preview" ${stale || !recommendation.action_type ? "disabled" : ""}>Preview response</button>${stale ? `<span class="nv-caveat">Feed is stale; response controls are disabled.</span>` : !recommendation.action_type ? `<span class="nv-caveat">No current containment recommendation.</span>` : ""}</div><div id="nv-action-preview-result"></div>`) +
